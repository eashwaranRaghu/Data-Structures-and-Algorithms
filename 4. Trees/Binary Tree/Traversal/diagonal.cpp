...
void dfs (TreeNode *root, int idx, map<int, Vi> &m) {
    ...
    dfs(root->left, idx-1, m); // key decrements on moving left
    dfs(root->right, idx, m); // key remains same on moving right as all the right childs are on same diagonal
}

void diagonal (TreeNode *root) {
    map<int, Vi> m; // map to store elements  against their diagonal number
